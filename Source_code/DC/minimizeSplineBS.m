function [X, fX, i] = minimizeSplineBS(X, f, length, varargin)
...
EXT = 10.0;                  % extrapolate maximum 3 times the current step-size
...
global globspl;
splines=varargin{1};
alldpoints=varargin{2};
% global EdatValue ElinValue EangValue EsegValue

% for iterative visualization
global detections glNhood

% global newLabeling outlierLabel TNeighbors frames
printEnergiesBS(0,X,varargin{:});
%         pause
...
%     if i==1
%         SPLXX=vecToBSplines(X,varargin{1});
%     end
%     if ~mod(i,2) || i==1
%         splines=vecToSplines(X,varargin{1});
% %         splines(1).coefs
% %         XXX'-X'
% %         pause
% %         XXX=X;
% %         isequal(SPLXX,splines)
% %         SPLXX=splines;
%         saveEnergySnapshot(varargin{2}, ...
%             detections, glNhood, varargin{6}, splines, varargin{4}, varargin{5}, 12);
%     end
    
    if ~mod(i,10) || i==1
        printEnergiesBS(i,X,varargin{:});
%         pause
%         fprintf('%s %6i;  Value %4.6e\r', S, i, f0);
%         fprintf('%4i|%8.1f|%8.1f|%8.1f|%6.1f|%6.1f|\n', ...
%             i,EdatValue, ElinValue, EangValue, EsegValue);
%     globspl(1).coefs(1,:)=X(1:4)';
%     globspl(1).coefs(2,:)=X(5:8)';
%     globspl(2).coefs(1,:)=X(9:12)';
%     globspl(2).coefs(2,:)=X(13:16)';
%     globspl(3).coefs(1,:)=X(17:20)';
%     globspl(3).coefs(2,:)=X(21:24)';
%     spxy=ppval(globspl,spl.start:spl.end);
% 
%     plot3(spxy(1,:),spxy(2,:),spl.start:spl.end,'-b');
%     reopenFig('optimization');
%     drawDCUpdate(globspl,1:numel(globspl),alldpoints,newLabeling,outlierLabel,TNeighbors,frames);
%     pause
