...
class trackingEvaluation(object):
    ...
    def __init__(self, t_sha, gt_path="./data/tracking", min_overlap=0.5, max_truncation = 0.15, mail=None):
        ...
        # set images that are generated for website
        self.www_img = {"0000": [10],\
                       }
        # works only from python 2.7
        # self.www_data = {k: [] for k in self.www_img.keys()}

        # backwards compatible to python 2.6.6
        self.www_data = dict((k,[]) for k in self.www_img.keys())
    ...
    def createEvalDir(self,cls):
        """Creates directory to store evaluation results and data for visualization"""
        self.eval_dir = os.path.join("./results/", self.t_sha, "eval", cls)
        if not os.path.exists(self.eval_dir):
            print "create directory:", self.eval_dir,
            os.makedirs(self.eval_dir)
            print "done"

    def loadGroundtruth(self,cls):
        """Helper function to load ground truth"""
        try:
            self._loadData(self.gt_path, cls=cls, loading_groundtruth=True)

        except IOError:
            return False
        return True

    def loadTracker(self, cls):
        """Helper function to load tracker data"""
        try:
            if not self._loadData(self.t_path, cls=cls, loading_groundtruth=False):
                return False
        except IOError:
            return False
        return True

    def _loadData(self, root_dir, cls="car", min_score=-1000, loading_groundtruth=False):
        """
        ...
        for seq, s_name in enumerate(self.sequence_name):
            ...
            for line in f:
                ...
                t_data.track_id_out = int(float(fields[1]))   # id
    ...
    def compute3rdPartyMetrics(self):
        ...
        for seq_idx in range(len(self.groundtruth)):
            ...
            #seq_trajectories = [[] for x in xrange(200)]
            ...
            seqfp = 0
            seqcost = 0
            # save mapping to file for required images
            # frame, gtID, trackingID, bbox(left,top,right,bottom)
            if "%04d" % seq_idx in self.www_img.keys():
                filename = os.path.join(self.eval_dir, "%s.txt" % self.sequence_name[seq_idx])
                # always overwrite
            ...
            for f in range(len(seq_gt)):
                ...
                for gg in g:
                    ...
                    #if gg.track_id >= len(seq_trajectories):
                        #seq_trajectories += [[] for x in xrange(max(100, gg.track_id-len(seq_trajectories)))]
                ...
                # check FN for truncation
                ignoredfn = 0
                nignoredtp = 0
                for gg in g:
                    if gg.tracker < 0 and gg.truncation>self.max_truncation:
                        gg.track_id_out = -99 # ignored FN
                        seq_ignored[gg.track_id][-1] = True
                        gg.ignored = True
                        ignoredfn += 1
                    elif gg.tracker>=0 and gg.truncation>self.max_truncation:
                        gg.track_id_out = -98 # ignored TP
                        seq_ignored[gg.track_id][-1] = True
                        gg.ignored = True
                        nignoredtp += 1
                    ...
                    if tt in last_ids[0]:
                        ...
                        if tid != lid and lid != -1 and tid != -1:
                            if g[i].truncation<self.max_truncation:
                                ...
                                g[i].track_id_out = -99 # ignored FN
                                #print g[i].track_id_out
                        #if lid != tid and lid != -1:
                ...
                tracked = []
                for gg in g:
                    tracked.append(gg.tracker)
                    if dump and f in self.www_img["%04d" % seq_idx]:
                        print>>dump, f, gg.track_id_out, gg.tracker, gg.id_switch, gg.fragmentation,\
                                gg.x1, gg.y1, gg.x2, gg.y2
                        self.www_data["%04d" % seq_idx].append((f, gg.track_id_out, gg.tracker,\
                                                                gg.id_switch, gg.fragmentation,\
                                                                gg.x1, gg.y1, gg.x2, gg.y2
                                                               ))
                for tt in t:
                    if tt.track_id in tracked:
                        continue
                    if tt.ignored:
                        dc_track = -99
                    else:
                        dc_track = tt.track_id
                    if dump and f in self.www_img["%04d" % seq_idx]:
                        print>>dump, f, -100, dc_track, 0, 0, tt.x1, tt.y1, tt.x2, tt.y2 
                        self.www_data["%04d" % seq_idx].append((f, -100, dc_track, 0, 0, tt.x1, tt.y1, tt.x2, tt.y2))
            ...
            if dump:
                dump.close()
            tmp_frag_debug.append(tmp_frags)
            # remove empty lists for current gt trajectories
            self.gt_trajectories[seq_idx] = seq_trajectories
            self.ign_trajectories[seq_idx] = seq_ignored
            # prepare data for sequence-based evaluation
            seq_results = OrderedDict(zip(("TP","FP","MISSES"),(seqtp,seqfp,seqfn)))
            if (seqtp+seqfn)>0 and (seqtp+seqfp)>0:
                precision = seqtp/float(seqtp+seqfp)
                seq_results["precision"] = precision
                recall = seqtp/float(seqtp+seqfn) 
                seq_results["recall"] = recall
                if (precision+recall)==0:
                    seq_results["F1"] = "n/a"
                else:
                    seq_results["F1"] = 2.*(precision*recall)/(precision+recall)
            else:
                seq_results["precision"] = "n/a"
                seq_results["recall"] = "n/a"
                seq_results["F1"] = "n/a"
            if seqtp==0 and seqfn==0:
                seq_results["MOTA"] = "n/a"
            else:
                seq_results["MOTA"]  = 1 - (seqfn + seqfp)/float(seqtp+seqfn)
            if seqtp==0: 
                seq_results["MOTP"] = "n/a"
            else:
                seq_results["MOTP"]  = seqcost / float(seqtp)
            self.seq_res.append(seq_results)
        ...
        for seq_idx, (seq_trajectories,seq_ignored) in enumerate(zip(self.gt_trajectories, self.ign_trajectories)):
            if len(seq_trajectories)==0:
                self.seq_res[seq_idx]["MT"] = "n/a"
                self.seq_res[seq_idx]["PT"] = "n/a"
                self.seq_res[seq_idx]["ML"] = "n/a"
                self.seq_res[seq_idx]["ID-SW"] = "n/a"
                self.seq_res[seq_idx]["FRAGS"] = "n/a"
                continue
            ...
            # save eval per sequence
            if (len(seq_trajectories)-n_ignored_tr)==0:
                self.seq_res[seq_idx]["MT"] = 0
                self.seq_res[seq_idx]["PT"] = 0
                self.seq_res[seq_idx]["ML"] = 0
            else:
                self.seq_res[seq_idx]["MT"] = tmpMT/float(len(seq_trajectories)-n_ignored_tr)
                self.seq_res[seq_idx]["PT"] = tmpPT/float(len(seq_trajectories)-n_ignored_tr)
                self.seq_res[seq_idx]["ML"] = tmpML/float(len(seq_trajectories)-n_ignored_tr)
            self.seq_res[seq_idx]["ID-SW"] = tmpId_switches
            self.seq_res[seq_idx]["FRAGS"] = tmpFragments
            self.seq_res[seq_idx]["Tracker Trajectories"] = self.n_tr_seq[seq_idx]
        ...
        # compute CLEARMOT
        # CLEARMOT definition for FP: non-associated object hypotheses, that are not a mismatch
        # here all non-associated object hypotheses are counted as FP and id-switches are counted seperately
        # therefore, MOTA is only 1- (FN+FP)/n_gt, with FP=FP_MOTA+ID-SWITCHES
        if self.n_gt==0:
            self.MOTA  = -float("inf")
            self.MODA = -float("inf")
        else:
            self.MOTA  = 1 - (self.fn + self.fp)/float(self.n_gt)
            self.MODA = 1-(self.fn+self.fp-self.id_switches)/float(self.n_gt)
        if self.tp==0:
            self.MOTP  = float("inf")
        else:
        ...
        if self.n_gt!=0:
            if self.id_switches==0:
                self.MOTAL = 1 - (self.fn + self.fp )/float(self.n_gt)
            else:
                self.MOTAL = 1 - (self.fn + self.fp - self.id_switches + math.log10(self.id_switches))/float(self.n_gt)
        else:
    ...
    def summary(self):
        ...
        mail.msg(self.printEntry("Multiple Object Precision Precision (MODP)", self.MODP))
        mail.msg("")
    ...
    def saveToStats(self,cls):
        self.summary()
        filename = os.path.join("./results", self.t_sha, "stats_%s.txt" % cls)
    ...
    def createImages(self,cls):
        for seq_idx, seq_data in self.www_img.iteritems():
            for frame in seq_data:
                ...
                for item in self.www_data[seq_idx]:
                    # item: f, gg.track_id, gg.tracker, gg.id_switch, gg.fragmentation, gg.x1, gg.y1, gg.x2, gg.y2
                    if item[0]!=frame:
                        continue
                    # get coordinates
                    x1 = item[-4]
                    ...
                    # set color
                    c = None
                    if item[2] >= 0 and item[1] == -100.: # false alarm
                        c = "red"
                    if item[1]>=0. and item[2] >= 0.:     # true positive
                        c = "green"
                    if item[1]>=0. and item[2] < 0.:          # missed ground truth
                        c = "blue"
                    # draw
                    draw.line(((x1,y1),(x2,y1)),fill=c,width=3)
                    draw.line(((x2,y1),(x2,y2)),fill=c,width=3)
                    draw.line(((x2,y2),(x1,y2)),fill=c,width=3)
                    draw.line(((x1,y2),(x1,y1)),fill=c,width=3)
                # save image
                filename = os.path.join("./results", self.t_sha, "eval", cls, "%s_%06d.jpg" % (seq_idx, frame))
                im.save(filename, "JPEG", quality=JPEG_QUALITY)
...
def evaluate(result_sha,mail):
    ...
    e = trackingEvaluation(t_sha=result_sha, mail=mail)
    classes = []
    for c in ("car", "pedestrian"):
        # load tracker data and check provided classes
        try:
            if not e.loadTracker(cls=c):
                continue
        ...
        if not e.loadGroundtruth(cls=c):
