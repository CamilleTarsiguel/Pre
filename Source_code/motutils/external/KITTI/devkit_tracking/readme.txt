...
This file describes the KITTI tracking benchmarks, consisting of 21 training
sequences and 29 test sequences. Despite the fact that we have labeled 8
different classes, only the
...
written using the matlab tools (readLabels.m) provided within this devkit. All
values (numerical or strings) are separated via spaces, each row corresponds to
one object. The 17 columns represent:
...
for Pedestrians due to their similarity in appearance. As evaluation criterion 
we follow the CLEARMOT and Mostly-Tracked/Partly-Tracked/Mostly-Lost metrics.
...
the tracker data in ./results/<sha_key>/data. Furthermore, evaluate_tracking.seqmap 
contains for every benchmark sequence the start and end frame. Ground truth for
training sequences can be downloaded separately. 
However, the ground truth for test is not available.
